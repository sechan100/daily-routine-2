...
interface MonthUnitControlProps {
  daysOfMonth: number[];
  onDaysOfMonthChange: (days: number[]) => void;
}
export const MonthUnitControl = ({
  daysOfMonth,
  onDaysOfMonthChange
}: MonthUnitControlProps) => {

  const toggleDay = useCallback((day: number) => {
    if (daysOfMonth.includes(day)) {
      onDaysOfMonthChange(daysOfMonth.filter(d => d !== day));
    }
    else {
      onDaysOfMonthChange([...daysOfMonth, day]);
    }
  }, [daysOfMonth, onDaysOfMonthChange]);
