 ...
 * @returns 
 */
const rebuildRoutineTree = (tree: RoutineTree, routineBuilder: RoutineTreeBuilder): RoutineTree => {
  const dependents = [
...
export const rippleRoutines = async (today: Day = Day.today()): Promise<void> => {
  ...
  for (const note of notes) {
    ...
    const newTree = rebuildRoutineTree(note.routienTree, routineBuilder);
