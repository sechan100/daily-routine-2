...
export const createNewSerializedRoutine = (routine: Routine): string => {
  return dedent`
  \n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n
  ---
 ...
 * @returns 
 */
export const serializeRoutine = (fileContent: string, routine: Routine) => {
  // %% daily-routine 이전 부분을 모두 가져옴.
  const userContent = fileContent.split(/%% daily-routine\s*\n```(?:json)?\s*[\s\S]*?\s*```\s*%%/)[0].trim();
  return dedent`
  ${userContent}
  %% daily-routine
  \`\`\`
 ...
 * @returns 
 */
export const deserializeRoutine = async (file: TFile): Promise<Routine> => {
