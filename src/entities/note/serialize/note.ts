...
import { NoteRoutineGroup, NoteRoutineLike, noteRoutineLikeTypeGuards } from "../note-routine-like";
import { Task } from "../task";
import { deserializeRoutine, deserializeRoutineGroup, serializeRoutineGroup } from "./routine";
...
export const serializeRoutineNote = (note: RoutineNote): string => {
  ...
  for (const routienLike of note.routineRoot) {
    if (noteRoutineLikeTypeGuards.isGroup(routienLike)) {
      routineGroups.push(routienLike);
    }
    else if (noteRoutineLikeTypeGuards.isRoutine(routienLike)) {
  ...
  return dedent`
    ...
    # Routines
    ${routineGroups.map(g => serializeRoutineGroup(g)).join('\n')}
  `;
}
...
export const deserializeRoutineNote = (day: Day, fileContent: string): RoutineNote => {
  ...
  const routineGroupBlock = fileContent.match(h2BlockRegex);
  const routineRoot: NoteRoutineLike[] = [];
  if (routineGroupBlock) {
    for (const block of routineGroupBlock) {
      if (block.startsWith("## UNGROUPED")) {
        const lines = block.replace("## UNGROUPED", '').trim().split('\n');
        for (const line of lines) {
          if (line.trim() === '') continue; // 빈 줄은 무시
          routineRoot.push(deserializeRoutine(line));
        }
      } else {
        const routineGroup = deserializeRoutineGroup(block);
