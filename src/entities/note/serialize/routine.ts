...
import { NoteRoutine, NoteRoutineGroup } from "../note-routine-like";
 ...
 * @returns '- [ ] [[name]]'
 */
export const serializeRoutine = (routine: NoteRoutine): string => {
 ...
 * @returns 
 */
export const deserializeRoutine = (routineLine: string): NoteRoutine => {
 ...
 * '과 같은 형식의 문자열
 */
export const serializeRoutineGroup = (group: NoteRoutineGroup): string => {
  return dedent`
    ## ${group.isOpen ? group.name : `(${group.name})`}
    ${group.routines.map(t => serializeRoutine(t)).join('\n')}
  `;
}
 ...
 * 과 같은 형식의 마크다운 블록
 */
export const deserializeRoutineGroup = (block: string): NoteRoutineGroup => {
  block = block.trim();
  ...
  return {
    ...
    routines: taskLines.map(l => deserializeRoutine(l)),
    isOpen
  };
}
